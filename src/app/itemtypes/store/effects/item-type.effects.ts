...
import { ItemTypeActionTypes,LoadStoriesByIdSuccess,LoadTopStoriesError, LoadTopStoriesSuccess } from '../actions/item-type.actions';
...
export class ItemTypeEffects {
    getStoriesDetails$ = createEffect(
      () => this.actions$.pipe(
        ...
        switchMap((action: any) => this.itemsDataService.getTopStoriesData()
          .pipe(
            map((topStoryIds:any) => {
              return new LoadTopStoriesSuccess(topStoryIds);
            }),
            catchError((errorMessage) => of(new LoadTopStoriesError({ error: errorMessage })))
          )),
      )
    );
  ...
  constructor(private actions$: Actions, private store: Store<any>, private itemsDataService: ItemsDataService) { }
